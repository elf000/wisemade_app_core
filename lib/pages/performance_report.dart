                ...
                child: Column(
                  children: [
                    ...
                    Container(
                        ...
                        child: ShareButton(screenshotController: screenshotController, performanceReport: state.performanceReport)
                    )
                  ]
                ),
              );
          ...
          MediaQuery(
            ...
            child: MaterialApp(
                ...
                home: Scaffold(
                    ...
                    body: SafeArea(
                     child: Content(loading: false, performanceReport: performanceReport, printMode: true)
                    )
                )
            )
          ),
          targetSize: Size(MediaQuery.of(context).size.width, MediaQuery.of(context).size.width * 2),
          pixelRatio: 1.5
        );
...
class Content extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final String formattedSince = loading ? '' : DateFormat('dd/MM/yyyy').format(DateTime.fromMillisecondsSinceEpoch(performanceReport?['since'] * 1000));
    ...
    final descriptionText = FlutterI18n.translate(context, 'performance_report.description', translationParams: { 'date': formattedSince });
    ...
    return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if(printMode == true) Container(
              margin: const EdgeInsets.fromLTRB(0, 50, 0, 20),
              child: Image.asset('images/logo-white.png', height: 60)
            ),
            if(printMode == true) Container(
                margin: const EdgeInsets.symmetric(vertical: 20),
                child: Text(descriptionText, style: Theme.of(context).textTheme.titleLarge, textAlign: TextAlign.center),
            ),
            SummaryBox(performanceReport: performanceReport, loading: loading, printMode: printMode),
...
class SummaryBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final String formattedSince = loading ? '' : DateFormat('dd/MM/yyyy').format(DateTime.fromMillisecondsSinceEpoch(performanceReport?['since'] * 1000));
    final String profitLabel = loading ? '' : performanceReport?['profitValue'] >= 0 ? 'Você ganhou' : 'Você perdeu';
    final String titleLabel = loading ? '' : performanceReport?['profitValue'] >= 0 ? 'valorização' : 'desvalorização';
    ...
    return Container(
      ...
      child: Column(
        children: [
          loading
            ? const SkeletonLine(style: SkeletonLineStyle(height: 30))
            : Text(
                title,
          ...
          if(printMode != true) Container(
            ...
            child: loading
                ...
                : RichText(
                    text: TextSpan(
                      children: [
                        TextSpan(text: '$profitLabel ', style: Theme.of(context).textTheme.titleMedium?.copyWith(color: Theme.of(context).colorScheme.shadow)),
                        TextSpan(text: Format.currency(performanceReport?['profitValue'], pattern: '$pricePrefix #,##0.00'), style: Theme.of(context).textTheme.titleMedium),
                      ]
                    ),
            )
          ),
...
class TopGainers extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return loading || topGainers.isNotEmpty ? Container(
      margin: const EdgeInsets.only(top: 20),
      child: Column(
          children: [
            Text(
              titleText,
              textAlign: TextAlign.center,
              style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Theme.of(context).colorScheme.shadow)
            ),
            Container(
              margin: const EdgeInsets.only(top: 20),
              child: Flex(
                  ...
                  children: topGainers.map<Widget>((gainer) {
                    ...
                    return CoinCard(coin: coin, priceVariation: gainer?['priceVariationPercentage'], loading: loading);
                  }).toList()
              )
            )
          ]
...
class TopLosers extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    List topGainers = performanceReport?['topLosers'] ?? [null, null, null];
            ...
            children: [
              ...
              Container(
                  ...
                  child: Flex(
                      direction: Axis.horizontal,
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: topGainers.map<Widget>((loser) {
                        final Coin? coin = loser == null ? null : Coin.fromJson(loser);

                        return CoinCard(coin: coin, priceVariation: loser?['priceVariationPercentage'], loading: loading);
                      }).toList()
                  )
              )
            ]
    ...
    return InkWell(
        ...
        child: Container(
            ...
            child: Column(
                children: [
                  loading
                      ? SkeletonAvatar(style: SkeletonAvatarStyle(height: 56, width: 56, borderRadius: BorderRadius.circular(50)))
                      : ClipRRect(
                    borderRadius: BorderRadius.circular(8.0),
                    child: Image.network(coin!.imageUrl, width: 56, height: 56),
                  ),
                  Container(
                    margin: const EdgeInsets.symmetric(vertical: 5),
                    child: loading
                        ? const SkeletonLine(style: SkeletonLineStyle(height: 15, width: 60))
                        : Text(coin!.formattedSymbol(), style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold)),
                  ),
                  loading
                      ? const SkeletonLine(style: SkeletonLineStyle(height: 10, width: 60))
